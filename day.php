...
function holidayDes($timestamp){
  $conn = new mysqli("localhost","mrbs", "mrbs-password",'mrbs');
  if ($conn->connect_error) {
      die("Connection failed: " . $conn->connect_error);
  }

  $sql = "select date,description from mrbs_holidays";
  ...
  while($row = mysqli_fetch_array($res)){
    $hd = date('Y-m-d',$row[0]);
    if($date == $hd){
      return "<br>".$row[1].", Holiday";
    }
  }
  return "";
}
include("read_timetable.php");
...
echo "<div id=\"dwm\">\n";
echo "<h2>" . utf8_strftime($strftime_format['date'], $timestamp).holidayDes($timestamp). "<br>".getEvent($timestamp)."</h2>\n";
if ($display_timezone)
{
